...
# If all will be downloaded, then set this value as 'None'
FILE_NAME = sys.argv[3]
# 4 args = SharePoint file name pattern
# If no pattern match files are required ot be downloaded, then set this value as 'None'
...
def get_files_by_pattern(pattern_keyword, folder):
    files_list = SharePoint()._get_files_list(folder)
    for file in files_list:
        if re.search(pattern_keyword, file.name):
            get_file(file.name, folder)
